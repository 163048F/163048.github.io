...
function startupCard() {
   for(var a=0;a<document.getElementsByClassName("cardAnimationDiv").length;){for(var b=document.getElementsByClassName("cardAnimationDiv")[a].children,c=0,d=b.length-1;c<b.length;)b[c].style.zIndex=d,1<c&&(b[c].style.display="none"),c++,d--;a++};
}
...
function goLeft(oriDiv) {
   ...
   if(a&&"cardSwipeDiv"===a.className){var b=parseInt(a.style.marginLeft.substring(0,a.style.marginLeft.length-2)),c=0;a.style.display="flex";a.style.zIndex=1;var d=setInterval(function(){1<=c?clearInterval(d):(0>b&&(b+=2),c+=.1,a.style.opacity=c,a.style.marginLeft=b+"px")},50);oriDiv.nextElementSibling&&(oriDiv.nextElementSibling.style.display="none");cardCurrent=a;cardCurrentIndex--};
   setCardIndicator();
}
